    ...
    for page in range(1, nb_pages + 1):
        ...
        for card in cards:
            try:
                ...
                brand_tag = card.find("h2", class_="listing-card_header_title mb-md-2 mb-0")
                ...
                price_tag = card.find("h3", class_="listing-card_header_price font-weight-bold text-uppercase mb-0")
...
if not API_KEY:
    st.error("❌ API key manquante. Ajoute GEMINI_API_KEY dans Streamlit → Settings → Secrets")
    st.stop()

genai.configure(api_key=API_KEY)

# IMPORTANT : utiliser -latest
